...
class MutableAtariEnv(gymnasium.Env, utils.EzPickle):
    """Gymnasium wrapper around the Arcade Learning Environment (ALE)."""
    ...
    def __init__(
        ...
        obs_type: Literal["rgb", "grayscale", "ram"] = "ram",
    ...
    def load_game(self) -> None:
        ...
        rom_path = roms.get_rom_path(self._game)
        if rom_path is None:
            raise error.Error(f"ROM for game {self._game} not found.")
    ...
    def reset(
        ...
        *,
        ...
        if seed is not None or mode is not None:
            self.load_game()
...
if __name__ == "__main__":
    ...
    env = MutableAtariEnv(game = "pacman", obs_type = 'ram', render_mode='human')#, mode =6)
    for mode in [0,6]:
        for episode in range(1):
            observation = env.reset(mode = mode)
            done = False
            terminal, truncation = False, False
            i = 0
            while not terminal and not truncation:
                i += 1
                env.render()
                # Using a random action for demonstration
                action = env.action_space.sample()
                observation, reward, terminal, truncation, metadata = env.step(action)
                if i == 100:
                    terminal = True
